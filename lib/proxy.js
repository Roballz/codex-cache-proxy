...
import { activeSession, applyIdentity } from './identity.js';
import { chatCompletionsToResponsesShape } from './cc_bridge.js';
import { DiagnosticsStore, UsageObserver } from './diagnostics.js';
import { copyResponseHeaders, forwardRequestHeaders } from './http.js';
import { applyReasoningSummary } from './reasoning.js';
...
function withActiveSession(config, patch) {
  return {
    ...config,
    sessions: config.sessions.map(session =>
      session.id === config.activeSessionId ? { ...session, ...patch } : session,
    ),
  };
}

function prepareGenerationBody(body, headers, config, upstream, path) {
  const session = activeSession(config);
  const chatMode = session?.chatMode ?? 'upstream';

  if (path !== '/v1/chat/completions' || chatMode !== 'bridge') {
    return {
      identityResult: applyIdentity(body, headers, config, upstream, path),
      bridge: { requested: false, active: false, fallback: false, reason: null },
    };
  }

  if (upstream.adapter !== 'sub2api') {
    const fallbackConfig = withActiveSession(config, { chatMode: 'upstream' });
    return {
      identityResult: applyIdentity(body, headers, fallbackConfig, upstream, path),
      bridge: {
        requested: true,
        active: false,
        fallback: true,
        reason: 'Responses Bridge currently supports only the Sub2API adapter.',
      },
    };
  }

  const converted = chatCompletionsToResponsesShape(body);
  if (!converted.bridged) {
    const fallbackConfig = withActiveSession(config, { chatMode: 'upstream' });
    return {
      identityResult: applyIdentity(body, headers, fallbackConfig, upstream, path),
      bridge: {
  ...
  const identityResult = applyIdentity(converted.body, headers, config, upstream, path);
  identityResult.diagnostics.bridge = {
    active: true,
    shape: 'responses',
    upstreamPath: path,
    originalMessageCount: Array.isArray(body.messages) ? body.messages.length : 0,
  };

  return {
    identityResult,
    bridge: { requested: true, active: true, fallback: false, reason: null },
  };
}
...
export function createProxy({ configStore, diagnostics = new DiagnosticsStore() }) {
  ...
  async function proxyGeneration(request, response, path, body) {
    ...
    const prepared = prepareGenerationBody(body, inboundHeaders, config, upstream, path);
    const identityResult = prepared.identityResult;
    identityResult.body = applyReasoningSummary(
      identityResult.body,
      config.reasoningSummaryEnabled === true,
    );
    const authorization = inboundHeaders.authorization || '';

    if (prepared.bridge.requested && !prepared.bridge.active) {
      identityResult.diagnostics.bridge = {
        active: false,
        fallback: true,
        reason: prepared.bridge.reason,
      };
    }
    ...
    const record = diagnostics.begin(
      identityResult.body,
      {
        ...
        mode: prepared.bridge.active ? `bridge:${identityResult.diagnostics.mode}` : identityResult.diagnostics.mode,
    ...
    try {
      ...
      if (prepared.bridge.active) response.setHeader('x-codex-cache-proxy-bridge', 'responses-shape');
      else if (prepared.bridge.fallback) response.setHeader('x-codex-cache-proxy-bridge', 'fallback');
      ...
      if (!upstreamResponse.body) {
        ...
        finishRecord(
          ...
          prepared.bridge.fallback
            ? `Responses Bridge fell back to native Chat Completions: ${prepared.bridge.reason}`
      ...
      let note = null;
      if (prepared.bridge.fallback) {
        note = `Responses Bridge fell back to native Chat Completions: ${prepared.bridge.reason}`;
      } else if (!observer.usage) {
        note = 'Upstream did not expose cache usage statistics in an observable response event.';
      } else if (downstreamClosedAfterTerminal) {
        note = 'Client closed after the normal terminal event; upstream tail was drained for diagnostics.';
      }

      finishRecord(diagnostics, record, observer, terminalState, note);
