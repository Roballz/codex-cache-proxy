...
export function createProxy({ configStore, diagnostics = new DiagnosticsStore() }) {
  async function proxyModels(request, response, path) {
    ...
    const timeout = setTimeout(() => controller.abort(new Error('timeout')), config.timeoutSeconds * 1000);
    ...
    try {
      ...
      response.setHeader('x-codex-cache-proxy-upstream', upstream.name);
      ...
      if (controller.signal.aborted) {
        if (!response.headersSent) throw new AppError(504, 'upstream_aborted', 'Upstream request was aborted or timed out.');
        response.destroy();
        return;
      }
  ...
  async function proxyGeneration(request, response, path, body) {
    ...
    const identityResult = applyIdentity(body, inboundHeaders, config, upstream);
    ...
    try {
      ...
      copyResponseHeaders(upstreamResponse.headers, response);
      response.setHeader('x-codex-cache-proxy-upstream', upstream.name);
      response.setHeader('x-codex-cache-proxy-identity-mode', config.identityMode);
      ...
      finishRecord(
        ...
        observer,
        terminalState,
        observer.usage ? null : 'Upstream did not expose cache usage statistics in an observable response event.',
      );
      response.end();
    } catch (error) {
      if (controller.signal.aborted) {
        const state = timedOut ? 'timeout' : clientDisconnected ? 'client_aborted' : 'aborted';
        finishRecord(diagnostics, record, observer, state, timedOut ? 'Upstream timeout.' : 'Request aborted.');
        if (!response.headersSent) {
          throw new AppError(timedOut ? 504 : 499, state, timedOut ? 'Upstream request timed out.' : 'Client disconnected.');
        }
        response.destroy();
        return;
      }
