...
function stripIdentity(body, headers) {
  const nextBody = { ...body };
  for (const key of ID_FIELDS) delete nextBody[key];
...
export function applyIdentity(body, headers, config, upstream, path = '') {
  ...
  if (
    ...
    config.chatCompletionsIdentityMode === 'upstream'
  ) {
    const stripped = stripIdentity(body, headers);
    return {
      ...stripped,
      changed: inbound.length > 0,
      diagnostics: {
        mode: 'upstream',
        source: 'upstream',
        inbound,
        outbound: inspectIdentity(stripped.body, stripped.headers),
        overwritten: inbound.length > 0,
        note: 'Chat Completions identity removed so the selected upstream can derive its own stable compatibility cache key.',
      },
    };
  }

  const mode = config.identityMode;
  ...
  const session = config.sessions.find(item => item.id === config.activeSessionId);
