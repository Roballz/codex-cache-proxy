...
export function defaults() {
  ...
  return {
    ...
    sessions: [{ id: profileId, name: 'Default RP', value: randomUUID() }],
    clientInterfaceMode: 'chat_completions',
    identityMode: 'lock',
...
export function validateConfig(input) {
  ...
  const clientInterfaceMode = input.clientInterfaceMode ?? 'chat_completions';
  requireValue(
    ['responses', 'chat_completions'].includes(clientInterfaceMode),
    'Invalid TT/ST interface mode.',
  );
  requireValue(['lock', 'fill', 'passthrough'].includes(input.identityMode), 'Invalid identity mode.');
  let chatCompletionsIdentityMode = input.chatCompletionsIdentityMode ?? 'upstream';
  // Legacy "global" was the experimental native-CC + forced identity mode.
  // It is no longer exposed because Responses Bridge is the supported locked-session CC path.
  if (chatCompletionsIdentityMode === 'global') chatCompletionsIdentityMode = 'upstream';
  requireValue(
    ['upstream', 'bridge'].includes(chatCompletionsIdentityMode),
    'Invalid Chat Completions mode.',
  );
  requireValue(
  ...
  const sessions = input.sessions.map(session => {
    assertKeys(session, ['id', 'name', 'value']);
    requireValue(
    ...
    return { id: session.id, name: normalizeName(session.name), value: session.value };
  });
